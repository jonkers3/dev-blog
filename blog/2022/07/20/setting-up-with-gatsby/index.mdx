...
In terms of initial setup, there are a few nice things to do, like:

Set up an alias for our components directory so we can use absolute imports (Typescript users will also need to update their `tsconfig.json`) :
...
If your setup is like mine, and you're using Typescript + CSS modules. If this is the case, you'll probably need a  definitions file along these lines to prevent the TS compiler from finding fault with our CSS module imports :

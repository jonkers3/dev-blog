...
But we don't plan on having an empty blog or markdown files with inconsistent metadata, so it isn't really necessary, but it does illuminate a bit how GraphQL queries work with frontmatter in Gatsby.
...
Obviously there is some kind of magic going on here. Gatsby takes the results of the `query` that gets exported and passes them right back into the component as a prop called `data`. GraphQL is perfect for this. Because the shape of the query is the shape of the data structure we are dereferencing in our component as we list out the posts.

...
GraphQL will throw an error: `Value "frontmatter___date" does not exist in "MdxFieldsEnum" enum`. The GraphQL query basically says `ORDER BY frontmatter___date`, and `frontmatter___date` doesn't exist in the GQL schema.

If `frontmatter___date` _did_ exist in any of our MDX files, it would be _added_ to the schema dynamically.

If we really wanted our query to work in this situation and return an empty result set instead of an error, we could do something like this.
...
This isn't necessary because we don't plan on having an empty blog or inconsistent metadata in our MDX files. It is mentioned simply to shed some light on how GraphQL queries work with frontmatter in Gatsby.

In any case, we need at least [one MDX file with a date](https://raw.githubusercontent.com/jonkers3/dev-blog/master/blog/2022/07/20/setting-up-with-gatsby/index.mdx) to exist in our blog folder. And it needs to be in a _subfolder_ of blog, as that is how we generate the slug.
...
Obviously there is some kind of magic going on here. Gatsby takes the results of the `query` that gets exported, and passes them into the component as a prop called `data`. We can see the shape of the data structure we are dereferencing in our component because it is the same shape as the query. GraphQL is great here also in that it  resembles ES6, making it all feel even more congruent.
...
Now that we have an index page, we want to be able to see the individual posts as well. We need a file to serve as our blog template:
